    ...
    fetch("http://localhost:3000/api/medic")
      ...
      .then((data) => {
        const groupedItems = data.reduce((acc, item) => {
          const numKeys = Object.keys(item).filter(
            (key) => key !== "_id" && key !== "_rev"
          ).length; // Exclude _id and _rev from count
          if (!acc[numKeys]) {
            acc[numKeys] = [];
          }
    ...
    return (
      <div className="mt-10 w-full overflow-x-scroll">
        <h3 className="text-xl font-bold mb-2">
          Items with {numKeys} key-value pairs
        </h3>
  ...
  return (
    <Router>
      <div className={"flex flex-col items-start justify-start"}>
        <nav
          className={
            "w-full top-0 flex justify-start sticky bg-gray-800 bg-opacity-90 text-white overflow-x-scroll"
          }
        >
          <ul
            className={
              "flex w-full bg-gray-200 bg-opacity-10 h-14 items-center p-4 justify-center gap-4"
            }
          >
